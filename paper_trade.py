...
Her calistirilisinda bir tur:
  ...
  5. Risk yonetimi: %5 risk/trade, max 4 acik pozisyon
  6. OKX'e market order + attached SL/TP gonder (dinamik leverage)
  7. Sonuclari paper_trade_log.json'a yaz

Kullanim:
   python paper_trade.py                   # gercek demo order
   python paper_trade.py --dry-run         # OKX'e baglanma
   python paper_trade.py --symbols BTC,ETH
"""
...
def detect_signal(symbol: str, tf: str, selected_models: Dict, fetcher,
...
def reconcile_positions(client, log: List[Dict], risk_pct: float) -> int:
    ...
    for entry in open_entries:
        ...
        candidates = [
            ...
            and h["open_ts"] >= entry_ts_ms - 60_000
            and h["close_ts"] > 0
        ]
        if not candidates:
            continue
...
def execute_signal(client, sig: Dict, balance: float, risk_pct: float,
    ...
    if dry_run:
        ...
        print(f"  [DRY] {inst_id} {side.upper()} size={size} lev={leverage}x "
    ...
    try:
        ...
        tg_send(
            f"\U0001f7e2 <b>OPEN</b>  {sig['symbol']} {sig['direction']}\n"
            f"Model: {sig['model']}\n"
...
def main(argv):
    ...
    if args.source == "mexc":
        if not args.dry_run:
            print("[ERR] --source mexc icin --dry-run zorunlu")
            return 1
        fetcher = MEXCFetcher()
    else:
        fetcher = OKXFetcher()
    ...
    if args.dry_run:
        ...
        print(f"Simule bakiye: {balance:.2f} USDT")
